...
(define (domain GAME)
  ...
  (:action almacenar
	     :parameters (?o - objeto ?z - zona)
	     :precondition (and (en ?o ?z) (en R ?z) (> (capacidad) 0))
	     :effect
	     (and
         (not (en ?o ?z))
		     (almacenado ?o)
         (decrease (capacidad) 1)
		   )
  )
  ...
  (:action dar
      ...
      (and
        ...
        (when (and (profesor ?p) (oscar ?o)) (increase (puntos) -3))
        (when (and (profesor ?p) (rosa ?o)) (increase (puntos) -4))
        (when (and (profesor ?p) (manzana ?o)) (increase (puntos) -5))
        (when (and (profesor ?p) (algoritmo ?o)) (increase (puntos) -10))
        (when (and (profesor ?p) (oro ?o)) (increase (puntos) -1))

        (when (and (principe ?p) (oscar ?o)) (increase (puntos) -1))
        (when (and (principe ?p) (rosa ?o)) (increase (puntos) -3))
        (when (and (principe ?p) (manzana ?o)) (increase (puntos) -4))
