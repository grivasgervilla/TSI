...
/*bool localPlanner(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan, costmap_2d::Costmap2DROS *costmap_ros){
    ...
    unsigned int nodo_actual = start_index;
    while(!camino_encontrado){
      vector<unsigned int> vecinos = findFreeNeighborCell(nodo_actual,costmap);

      for(int i = 0; i < vecinos.size(); i++){
        if( distancia(vecinos.at(i), indice_goal) < min_distancia){
          proximo_nodo = i;
          min_distancia = distancia(vecinos.at(i), indice_goal);
        }
      }
